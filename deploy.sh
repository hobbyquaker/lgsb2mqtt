...
#
# Optional env vars:
#   REMOTE_DIR   (default: /usr/local/lib/node_modules/lgsb2mqtt)
#   REMOTE_TMP   (default: /tmp)
#   SERVICE      systemd unit(s) to restart after deploying; default: every active
#                lgsb2mqtt@<name> instance plus a plain lgsb2mqtt unit if present

REMOTE_HOST="${1:-mqtt-ifaces}"
REMOTE_DIR="${REMOTE_DIR:-/usr/local/lib/node_modules/lgsb2mqtt}"
...
sudo ln -sfn "$REMOTE_DIR/index.js" /usr/local/bin/lgsb2mqtt
if [[ -z "$SERVICE" ]]; then
  SERVICE="$(systemctl list-units --plain --no-legend --type=service 'lgsb2mqtt@*.service' lgsb2mqtt.service | awk '{print $1}' | tr '\n' ' ')"
fi
if [[ -n "$SERVICE" ]]; then
  ...
  echo "No lgsb2mqtt service found to restart (install one with: sudo lgsb2mqtt --install ...)"
fi
