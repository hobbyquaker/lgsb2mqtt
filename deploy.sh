...
#   sudo lgsb2mqtt --install -n soundbar -a 192.168.1.50 -u mqtt://broker
...
if [[ -n "$SERVICE" ]]; then
  ...
  echo "No $PKG service found to restart (install one with: sudo $PKG --install -n soundbar -a 192.168.1.50 -u mqtt://broker)"
fi
