...
export function indexToName(names, index) {
    return names[index] !== undefined ? names[index] : 'unknown_' + index;
}

export function nameToIndex(names, value) {
 ...
 */
export function statusFor(msg, key, value, ranges = {}) {
 ...
 */
export function commandFor(item, value, ranges = {}) {
...
export function rangeBoundFor(msg, key) {
    const entry = byRaw.get(msg + '/' + key);
    if (!entry) {
        return null;
    }
